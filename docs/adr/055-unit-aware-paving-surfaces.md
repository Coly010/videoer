...
Construction validity is measured rather than inferred from mesh validity. The report records plan coverage, skipped boundary-cell count and maximum skipped span, and the minimum clearance between every tilted/settled unit top and the continuous joint plane. Generation fails when coverage drops below the host contract, a boundary omission is too wide, or any stone—including repair-patch stones—can disappear beneath the joint bed. Narrow clipped boundary cells are merged or retained as cut units instead of being silently discarded. These invariants prevent material debugging from concealing a construction failure.

Large-scale correlated weathering remains the existing material macro-variation layer. This first slice deliberately does not invent renderer-specific per-island randomness or hundreds of duplicate materials. If independent per-unit colour/roughness scalars become necessary after visual review, they will enter the renderer-independent geometry attribute model with stable unit IDs and explicit interpolation semantics.
