...
Any Poly Haven ingestion must reuse the normalized open-source-material boundary rather than bypassing it. Persist provider asset identity, retrieval date, raw licence evidence, exact files and hashes, physical dimensions, channel semantics, colour spaces, normal convention and rendered scale probes. Poly Haven reconciled its API-service terms on 2026-07-18; ADR 067 records the exact reviewed commit and fail-closed live provider-file adapter. Live use requires the Videoer User-Agent and visible Poly Haven credit, while exact offline reuse remains governed by the downloaded asset's recorded CC0 evidence. Rendering never contacts the provider. A changed terms hash blocks live acquisition pending another explicit review.
