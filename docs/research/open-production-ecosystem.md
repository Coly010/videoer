...
- [Poly Haven API](https://polyhaven.com/our-api) and [API terms](https://github.com/Poly-Haven/Public-API/blob/master/ToS.md): the current public API is free for commercial use. Live requests require a product-identifying User-Agent or Referer, and a product surfacing live API content must identify Poly Haven. This is an API-service condition; downloaded/self-hosted CC0 assets do not acquire an attribution requirement.
