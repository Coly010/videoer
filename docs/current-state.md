...
| Blender Cloth (built-in) + `render_cloth_walk.py` | Clothing on the walking human (ADR 076): fitted (armature-deform) / loose (cloth sim) / hybrid garments | Adopted. Verified across 10 garments (dress, pyjamas, shirt/tie/trousers, mini-skirt/crop-top, sweater/jeans) + 2 assembled outfits on the walk, with a headless mechanical gate + visual contact sheets. Not yet wired into the campaign pipeline (follow-up). |
