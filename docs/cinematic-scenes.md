...
- `camera-shot-intent` binds a typed editorial brief to the same executable camera sampler. The brief names its purpose and movement grammar (`locked-off`, push/pull, lateral, vertical, or free), explicit sampled speed and acceleration limits, and one or more image framing gates. The generic verifier measures the actual sampled path; the render gates continue to prove subject coverage/framing from pixels. This avoids treating camera-to-target distance as a false proxy for composition.
