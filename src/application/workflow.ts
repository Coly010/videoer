...
import { loadCampaignState, nextRenderRevision, saveCampaignState, type RenderRevision } from '../domain/state.js';
...
export async function renderCampaign(campaignFile: string, options: { kind: 'draft' | 'final'; changes?: string[] }) {
  const ctx = context(campaignFile);
  const [campaign, storyboard, state] = await Promise.all([loadCampaign(ctx.campaignFile), loadStoryboard(ctx.paths.storyboard), loadCampaignState(ctx.paths.state)]);
  const revision = nextRenderRevision(state, { kind: options.kind, path: '', changes: options.changes ?? [] });
  ...
  const output = await render(createRenderPlan(campaign, storyboard, ctx.campaignFile), { outputPath, draft: options.kind === 'draft' });
...
function findRevision(renders: RenderRevision[], requested = 'latest') {
  const revision = requested === 'latest' ? renders.at(-1) : renders.find((item) => item.id === requested);
  if (!revision) throw new Error(requested === 'latest' ? 'Campaign has no renders' : `Render '${requested}' does not exist`);
  return revision;
}

export async function inspectRender(campaignFile: string, requested = 'latest') {
  const ctx = context(campaignFile);
  const [storyboard, state] = await Promise.all([loadStoryboard(ctx.paths.storyboard), loadCampaignState(ctx.paths.state)]);
  ...
  const frames = await Promise.all(storyboard.shots.map(async (shot) => {
  ...
  if (!state.inspections.includes(inspectionRef)) await saveCampaignState(ctx.paths.state, { ...state, inspections: [...state.inspections, inspectionRef] });
...
export async function verifyRender(campaignFile: string, requested = 'latest') {
  const ctx = context(campaignFile);
  const [campaign, state] = await Promise.all([loadCampaign(ctx.campaignFile), loadCampaignState(ctx.paths.state)]);
  const revision = findRevision(state.renders, requested);
  const renderPath = resolve(ctx.root, revision.path);
  const result = await verifyVideo(renderPath, campaign, revision);
  const reportPath = join(ctx.paths.reports, `${revision.id}.json`);
  await writeJson(reportPath, { schemaVersion: 1, render: revision, result, createdAt: new Date().toISOString() });
  const reportRef = relative(ctx.root, reportPath);
  if (!state.verificationReports.includes(reportRef)) await saveCampaignState(ctx.paths.state, { ...state, verificationReports: [...state.verificationReports, reportRef] });
  return { revision, reportPath, ...result };
}

export async function reviseShot(campaignFile: string, shotId: string, change: { text?: string; caption?: string; motion?: MotionPreset }) {
  const ctx = context(campaignFile);
  ...
  if (!change.text && !change.caption && !change.motion) throw new Error('A shot revision requires text, caption, or motion');
  const shots = [...storyboard.shots];
  const current = shots[index]!;
  shots[index] = { ...current, ...change, generation: { ...current.generation, revision: current.generation.revision + 1, stale: current.sources.some((source) => source.kind === 'generated') } };
  await saveStoryboard(ctx.paths.storyboard, { ...storyboard, shots });
  return { shotId, revision: shots[index]!.generation.revision, staleGeneratedAssets: shots[index]!.generation.stale, changed: Object.keys(change) };
}
